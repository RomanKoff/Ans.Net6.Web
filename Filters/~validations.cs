...
namespace Ans.Net6.Web.Filters
{
        ...
        : ValidationAttribute
    {
            ...
            ValidationContext validationContext)
        {
            ...
            if ((MinLength > 0) && (v1.Length < MinLength))
                return new ValidationResult(
                    _ResValidation.Template_LengthLessMin.TemplateFor(MinLength));
            if ((MaxLength > 0) && (v1.Length > MaxLength))
                return new ValidationResult(
                    _ResValidation.Template_LengthExceededMax.TemplateFor(MaxLength));
        ...
        : ValidationAttribute
    {
            ...
            ValidationContext validationContext)
        {
            ...
            if (v1 < Min)
                return new ValidationResult(
                    _ResValidation.Template_NumberLessMin.TemplateFor(Min));
            if (v1 > Max)
                return new ValidationResult(
                    _ResValidation.Template_NumberExceededMax.TemplateFor(Max));
            return ValidationResult.Success;
        }
    }
